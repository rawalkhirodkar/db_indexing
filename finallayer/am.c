...
/* splits a leaf node */
AM_BTreeSplitLeaf(fileDesc,pageBuf,pageNum,attrLength,recId,value,status,index,key)
...
char *key; /* returns the key to be filled in the parent */
{
	...
	AM_BTreeCompact(1,(header->numKeys)/2,pageBuf,tempPage,header);
	...
	AM_BTreeCompact((header->numKeys)/2 + 1,header->numKeys
	...
	if (index <= ((header->numKeys)/2))
	{
		...
		errVal = AM_BTreeInsertintoLeaf(tempPage,attrLength,value,recId,index,status);
	}
	else
	{
		/* value to be inserted in second half */
		index = index - ((header->numKeys)/2);
		errVal = AM_BTreeInsertintoLeaf(tempPageBuf,attrLength,value,
					   recId,index,status);
	}
	...
	if ((*pageNum) == AM_RootPageNum)
	{
		...
		AM_BTreeFillRootPage(pageBuf,tempPageNum1,tempPageNum,key,
...
AM_BTreeFillRootPage(pageBuf,pageNum1,pageNum2,value,attrLength,maxKeys)
...
AM_BTreeAddtoParent(fileDesc,pageNum,value,attrLength)
int fileDesc;
...
int attrLength;
{
	...
	if ((header->numKeys) < (header->maxKeys))
	{
		...
		AM_BTreeAddtoIntPage(pageBuf,value,pageNum,header,offset);
	...
	else
	{
		...
		AM_BTreeSplitIntNode(pageBuf,tempPage,pageBuf1,header,
		...
		if (pageNumber == AM_RootPageNum)
		{
			...
			attribute value */
			AM_BTreeFillRootPage(pageBuf,pageNum2,pageNum1,value,
			header->attrLength, header->maxKeys);
		...
		else
		{
			...
			internal node*/
			errVal =  AM_BTreeAddtoParent(fileDesc,pageNum1,value,
						 attrLength);
			AM_Check;
		}
	}
...
AM_BTreeSplitIntNode(pageBuf,pbuf1,pbuf2,header,value,pageNum,offset)
...
/* splits a leaf node */
AM_ListSplitLeaf(fileDesc,pageBuf,pageNum,attrLength,recId,value,status,index)
int fileDesc; /* file descriptor */
char *pageBuf; /* pointer to buffer */
int *pageNum; /* pagenumber of new leaf created */
int attrLength; 
...
int index; /* place where key is to be inserted */
{
	// int t;
	// bcopy(value,&t,AM_si);
	// printf("gonna splits on value %d \n",t);
	AM_NODE head,temphead; /* local header */
	AM_NODE *header,*tempheader;
	char tempPage[PF_PAGE_SIZE]; /* temporary page for manipulation on the page */
	char *tempPageBuf,*tempPageBuf1;/* buffers for new pages to be allocated */
	int errVal; 
	int tempPageNum,tempPageNum1;/* pagenumbers for pages to be allocated */
	...
	/* copy header from buffer */
	bcopy(pageBuf,header,AM_sn);
	// printf("printing header details:\n");
	// AM_PrintLeafNode(pageBuf,'i');
	// /* compact half the keys into temporary page */
	AM_ListCompact(1,(header->numKeys)/2,pageBuf,tempPage,header);
	/* Allocate a new page for the other half of the leaf*/
	errVal = PF_AllocPage(fileDesc,&tempPageNum,&tempPageBuf);
	AM_Check;

	/* compact the other half keys */
	AM_ListCompact((header->numKeys)/2 + 1,header->numKeys
			      ,pageBuf,tempPageBuf,header);
	...
	{
		/*value to be inserted is in first half */
		errVal = AM_ListInsertintoLeaf(tempPage,attrLength,value,recId,
					   index,status);
	}
	else
	{
		/* value to be inserted in second half */
		index = index - ((header->numKeys)/2);
		errVal = AM_ListInsertintoLeaf(tempPageBuf,attrLength,value,
					   recId,index,status);
	}

	/* change the next leafpage of first half of leaf to second half */
	bcopy(tempPage,tempheader,AM_sn);
	tempheader->nextLeafPage = tempPageNum;
	bcopy(tempheader,tempPage,AM_sn);
	bcopy(tempPage,pageBuf,PF_PAGE_SIZE);

	bcopy(tempPageBuf,tempheader,AM_sn);
	tempheader->previousLeafPage = *pageNum;
	bcopy(tempheader,tempPageBuf,AM_sn);	
	...
	//AM_DumpLeafPages(fileDesc,0,'i',attrLength);
	//printf("printing pageBuf details:\n");
	//AM_PrintLeafNode(pageBuf,'i');
	//printf("printing tempPageBuf details:\n");
	//AM_PrintLeafNode(tempPageBuf,'i');
}


AM_ListDeletePage(fileDesc,pageNum)
int fileDesc; /* file descriptor */
int *pageNum;
{
	AM_NODE head; /* local header */
	AM_NODE *header;
	header = &head;
	...
	if(header->numKeys == 0){
		if(*pageNum == AM_HeadPageNum && header->nextLeafPage == AM_NULL_PAGE){
			//printf("CASE1\n");
		}
		else if(*pageNum == AM_HeadPageNum && header->nextLeafPage != AM_NULL_PAGE){
			//printf("CASE2\n");

			AM_NODE thead; /* local header */
			AM_NODE *theader;
			theader = &thead;
			char *tpageBuf;
			int tpageNum;

			tpageNum = header->nextLeafPage;
			errVal = PF_GetThisPage(fileDesc,tpageNum,&tpageBuf);
			AM_Check;

			bcopy(tpageBuf,theader,AM_sn);
			theader->previousLeafPage = AM_NULL_PAGE;
			AM_HeadPageNum = tpageNum;
			bcopy(theader,tpageBuf,AM_sn);

			errVal = PF_UnfixPage(fileDesc,tpageNum,TRUE);
			AM_Check;

			errVal = PF_DisposePage(fileDesc,*pageNum);
			AM_Check;
		}
		else if(*pageNum != AM_HeadPageNum && header->nextLeafPage != AM_NULL_PAGE){
			//printf("CASE3\n");

			AM_NODE thead1,thead2; /* local header */
			AM_NODE *theader1,*theader2;
			theader1 = &thead1;
			theader2 = &thead2;
			char *tpageBuf1, *tpageBuf2;
			int tpageNum1, tpageNum2;

			tpageNum1 = header->previousLeafPage;
			tpageNum2 = header->nextLeafPage;
			errVal = PF_GetThisPage(fileDesc,tpageNum1,&tpageBuf1);
			AM_Check;
			errVal = PF_GetThisPage(fileDesc,tpageNum2,&tpageBuf2);
			AM_Check;

			bcopy(tpageBuf1,theader1,AM_sn);
			bcopy(tpageBuf2,theader2,AM_sn);
			theader1->nextLeafPage = tpageNum2;
			theader2->previousLeafPage = tpageNum1;
			bcopy(theader1,tpageBuf1,AM_sn);
			bcopy(theader2,tpageBuf2,AM_sn);

			errVal = PF_UnfixPage(fileDesc,tpageNum1,TRUE);
			AM_Check;
			errVal = PF_UnfixPage(fileDesc,tpageNum2,TRUE);
			AM_Check;
			...
			tpageNum = header->previousLeafPage;
			errVal = PF_GetThisPage(fileDesc,tpageNum,&tpageBuf);
			AM_Check;

			bcopy(tpageBuf,theader,AM_sn);
			theader->nextLeafPage = AM_NULL_PAGE;
			bcopy(theader,tpageBuf,AM_sn);
