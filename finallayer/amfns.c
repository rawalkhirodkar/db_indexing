...
/* Creates a secondary index file for B+ tree called fileName.indexNo */
AM_BTreeCreateIndex(fileName,indexNo,attrType,attrLength)
...
AM_ListCreateIndex(fileName,indexNo,attrType,attrLength)
char *fileName;/* Name of indexed file */
int indexNo;/*number of this index for file */
char attrType;/* 'c' for char ,'i' for int ,'f' for float */
int attrLength; /* 4 for 'i' or 'f', 1-255 for 'c' */
{
	...
	/* initialise the header */
	header->nextLeafPage = AM_NULL_PAGE;
	header->previousLeafPage = AM_NULL_PAGE;
	header->recIdPtr = PF_PAGE_SIZE;
	header->keyPtr = AM_sn;
	header->freeListPtr = AM_NULL;
	header->numinfreeList = 0;
	...
	/* copy the header onto the page */
	bcopy(header,pageBuf,AM_sn); // check what it is ?
	...
	/* initialise the root page and the leftmost page numbers */
	AM_HeadPageNum = pageNum;
	return(AME_OK);
}


/* Destroys the Btree index fileName.indexNo */
AM_BTreeDestroyIndex(fileName,indexNo)
char *fileName;/* name of indexed file */
int indexNo; /* number of this index for file */
{
...
/* Destroys the List index fileName.indexNo */
AM_ListDestroyIndex(fileName,indexNo)
char *fileName;/* name of indexed file */
int indexNo; /* number of this index for file */
{
	char indexfName[AM_MAX_FNAME_LENGTH];
...
/* Inserts a value,recId pair into the Btree */
AM_BTreeInsertEntry(fileDesc,attrType,attrLength,value,recId)
int fileDesc; /* file Descriptor */
...
int recId; /* recId to be inserted */
{
	...
	status = AM_BTreeSearch(fileDesc,attrType,attrLength,value,&pageNum,&pageBuf,&index);
	...
	inserted = AM_BTreeInsertintoLeaf(pageBuf,attrLength,value,recId,index,status);
	...
	{
		/* Split the leaf page */
		addtoparent = AM_BTreeSplitLeaf(fileDesc,pageBuf,&pageNum,
		...
		if (addtoparent == TRUE)
		{
			errVal = AM_BTreeAddtoParent(fileDesc,pageNum,key,attrLength);
			if (errVal < 0)
			{
				AM_EmptyStack();
				AM_Errno = errVal;
...
/* Inserts a value,recId pair into the list */
AM_ListInsertEntry(fileDesc,attrType,attrLength,value,recId)
int fileDesc; /* file Descriptor */
char attrType; /* 'i' or 'c' or 'f' */
int attrLength; /* 4 for 'i' or 'f', 1-255 for 'c' */
char *value; /* value to be inserted */ 
int recId; /* recId to be inserted */
{
	...
	/* Search the leaf for the key */
	status = AM_ListSearch(fileDesc,attrType,attrLength,value,&pageNum,
			   &pageBuf,&index);
	//printf("PAGENUM:%d INDEX:%d\n",pageNum,index);
	...
	/* Insert into leaf the key,recId pair */
	inserted = AM_ListInsertintoLeaf(pageBuf,attrLength,value,recId,index,status);
	//printf("INSERTED : %d\n",inserted);
...
becomes empty */
AM_BTreeDeleteEntry(fileDesc,attrType,attrLength,value,recId)
int fileDesc; /* file Descriptor */
...
int recId; /* id of the record to delete */
{
	...
	there */
	status = AM_BTreeSearch(fileDesc,attrType,attrLength,value,&pageNum,
			   &pageBuf,&index);
...
/* Deletes the recId from the list for value and deletes value if list
becomes empty */
AM_ListDeleteEntry(fileDesc,attrType,attrLength,value,recId)
int fileDesc; /* file Descriptor */
char attrType; /* 'c' , 'i' or 'f' */
int attrLength; /* 4 for 'i' or 'f' , 1-255 for 'c' */
char *value;/* Value of key whose corr recId is to be deleted */
int recId; /* id of the record to delete */
{
	...
	short temp; 
	char *currRecPtr;/* pointer to the current record in the list */
	AM_NODE head,*header;/* header of the page */
	int recSize; /* length of key,ptr pair for a leaf */
	int tempRec; /* holds the recId of the current record */
	int errVal; /* holds the return value of functions called within 
				                            this function */
	...
	recSize = attrLength + AM_ss;
	currRecPtr = pageBuf + AM_sn + (index - 1)*recSize + attrLength;
	bcopy(currRecPtr,&nextRec,AM_ss);
	...
	while(nextRec != 0)
	{
		...
		if (recId == tempRec)
		{
			// // VERIFY BY SOMEONE //
			bcopy(pageBuf + nextRec + AM_si,currRecPtr,AM_ss);
			// if( strncmp(&nextRec, pageBuf + AM_sl + (index - 1)*recSize + attrLength, AM_ss) == 0 ){
			// 	//printf("HA\n");
			// 	bcopy(currRecPtr,pageBuf + AM_sl + (index - 1)*recSize + attrLength, AM_ss); 
			// }
			// else{
			// 	//printf("HAHA\n");
			// 	bcopy(currRecPtr, pageBuf + nextRec - AM_ss,AM_ss); 
			// }
			//////////////////////////////////////////////////////////////////
			/* Delete recId */
			header->numinfreeList++;
	...
	/* check if list is empty */
	bcopy(pageBuf + AM_sn + (index - 1)*recSize + attrLength,&temp,AM_ss);
	if (temp == 0)
	{
		/* list is empty , so delete key from the list */
		for(i = index; i < (header->numKeys);i++)
			bcopy(pageBuf + AM_sn + i*recSize,pageBuf + AM_sn + 
				(i-1)*recSize,recSize);
		header->numKeys--;
		header->keyPtr = header->keyPtr - recSize;
	}
