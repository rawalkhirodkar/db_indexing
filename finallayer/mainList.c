...
double t1,t2, itime = 0, dtime = 0, itime2 = 0, dtime2 = 0;	
	...
	for (recnum=0; recnum < 20; recnum++){
		AM_ListInsertEntry(fd,INT_TYPE,sizeof(int),(char *)&recnum,
				recnum);
	}

	printf("deleting odd number records\n");
	for (recnum=1; recnum < 20; recnum += 2)
		AM_ListDeleteEntry(fd,INT_TYPE,sizeof(int),(char *)&recnum,
					recnum);

	printf("retrieving even number records\n");
	numrec= 0;
	sd = AM_ListOpenIndexScan(fd,INT_TYPE,sizeof(int),EQ_OP,NULL);
	...
	for (recnum=0; recnum < maxrecs; recnum +=1){
		for(i=0;i<10;i++){
			...
			AM_ListDeleteEntry(fd,INT_TYPE,sizeof(int),(char *)&recnum,rec);
	...
	/* create index */
	printf("creating index\n");
	AM_ListCreateIndex(NRLNAME,0,INT_TYPE,sizeof(int));
	...
	for (recnum=0; recnum < 10; recnum++){
		...
		AM_ListInsertEntry(fd,INT_TYPE,sizeof(int),(char *)&tmp,
				recnum);
	}
	...
	for (recnum=1; recnum < 10; recnum += 2){
		tmp = (recnum*10 + recnum);
		AM_ListDeleteEntry(fd,INT_TYPE,sizeof(int),(char *)&tmp,
					recnum);
	}
	...
	for (recnum=0; recnum < maxrecs; recnum +=1){
		for(i=0;i<10;i++){
			...
			tmp = (recnum*10 + i);
			AM_ListDeleteEntry(fd,INT_TYPE,sizeof(int),(char *)&tmp,rec);
