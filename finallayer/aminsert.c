...
int status;/* Whether key is a new key or an old key */
{
		...
		/* key is already present */ 
	{
		...
		AM_BTreeInsertToLeafFound(pageBuf,recId,index,header);
	...
	if ((header->freeListPtr) == 0)
		...
		{    
			AM_BTreeInsertToLeafNotFound(pageBuf,value,recId,index,
						header);
			header->numKeys++;
			bcopy(header,pageBuf,AM_sl);
			return(TRUE);
	...
	{
		AM_BTreeInsertToLeafNotFound(pageBuf,value,recId,index,header);
		header->numKeys++;
		bcopy(header,pageBuf,AM_sl);
		return(TRUE);
	}
	...
	{
		/* Compact the freelist so that we get enough space in the middle 
		so that the new key can be inserted */
		AM_BTreeCompact(1,header->numKeys,pageBuf,tempPage,header);
...
AM_BTreeCompact(low,high,pageBuf,tempPage,header)
int low;
int high;
char *pageBuf;
char *tempPage;
...
/* Inserts a key into a leaf node */
AM_ListInsertintoLeaf(pageBuf,attrLength,value,recId,index,status)
char *pageBuf;/* buffer where the leaf page resides */
int attrLength;
char *value;/* attribute value to be inserted*/
int recId;/* recid of the attribute to be inserted */
...
int status;/* Whether key is a new key or an old key */
{
		...
		/* key is already present */ 
	{
		...
		/* insert into leaf - no need to split */
		AM_ListInsertToLeafFound(pageBuf,recId,index,header);
	...
	if ((header->freeListPtr) == 0)
		...
		{    
			AM_ListInsertToLeafNotFound(pageBuf,value,recId,index,
						header);
			header->numKeys++;
			bcopy(header,pageBuf,AM_sn);
			return(TRUE);
	...
	if ((header->recIdPtr - header->keyPtr) > recSize)
	{
		AM_ListInsertToLeafNotFound(pageBuf,value,recId,index,header);
		header->numKeys++;
		bcopy(header,pageBuf,AM_sn);
		return(TRUE);
	}
	...
	/*there is enough space in the freelist and in the middle put together */
	{
		/* Compact the freelist so that we get enough space in the middle 
		so that the new key can be inserted */
		AM_ListCompact(1,header->numKeys,pageBuf,tempPage,header);
...
AM_NODE *header;
{
	...
	/* save  the old head of recId list */
	bcopy(pageBuf+AM_sn+(index-1)*recSize + header->attrLength,
	      (char *)&oldhead, AM_ss);

        /* Update the head of recId list to the new recid to be added */
	bcopy((char *)  &tempPtr,pageBuf+AM_sn + (index-1)*recSize + 
	       header->attrLength,AM_ss);
...
AM_NODE *header;
{
	...
	/* create space for the new key by pushing keys greater than that to 
								    the right */
	for(i = header->numKeys;i >= index; i--)
		{bcopy(pageBuf+AM_sn+(i-1)*recSize,pageBuf+AM_sn+i*recSize,recSize);
	 } 
	/* Update the header */
	header->keyPtr = header->keyPtr + recSize;
	
	/* copy the new key */
	bcopy(value,pageBuf+AM_sn+(index-1)*recSize,header->attrLength);
	
	/* make the head of list NULL*/
	bcopy((char *)&null,pageBuf+AM_sn+(index-1)*recSize+
	       header->attrLength,AM_ss);
...
be space in the middle for a new key. This compacts all the recid's to the right
so that there is enough space in the middle */
AM_ListCompact(low,high,pageBuf,tempPage,header)
int low;
int high;
char *pageBuf;
char *tempPage;
AM_NODE *header;
{
	...
	for (i = low, j = 1; i <= high; i++,j++)
	{
		offset1 = (i - 1) * recSize + AM_sn;
		offset2 = (j - 1) * recSize + AM_sn;
		bcopy(pageBuf + offset1,tempPage + offset2,header->attrLength);
		bcopy(pageBuf + offset1 + header->attrLength,(char *)&nextRec
		      ,AM_ss);
		bcopy((char *)&recIdPtr,tempPage + offset2 + header->attrLength,
	...
	/* Initialise the header appropriately */
	tempheader->nextLeafPage = header->nextLeafPage;
	tempheader->previousLeafPage = header->previousLeafPage;
	tempheader->recIdPtr = recIdPtr + AM_si + AM_ss;
	tempheader->keyPtr = offset2 + recSize;
	tempheader->freeListPtr = 0;
	tempheader->numinfreeList = 0;
