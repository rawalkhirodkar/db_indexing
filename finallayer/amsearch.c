...
be inserted */
AM_BTreeSearch(fileDesc,attrType,attrLength,value,pageNum,pageBuf,indexPtr)
int fileDesc;
char attrType;
int attrLength;
char *value;
                                                            ...
                                                            can be inserted */
{
	...
	while ((**pageBuf) != 'l')
	{
		...
		nextPage = AM_BTreeBinSearch(*pageBuf,attrType,attrLength,value,indexPtr,iheader);
	...
	return(AM_BTreeSearchLeaf(*pageBuf,attrType,attrLength,value,indexPtr,lheader));
}


/* Finds the place (index) from where the next page to be followed is got*/
AM_BTreeBinSearch(pageBuf,attrType,attrLength,value,indexPtr,header)
...
be inserted */
AM_BTreeSearchLeaf(pageBuf,attrType,attrLength,value,indexPtr,header)
...
/* searches for a key in a list - returns FOUND or NOTFOUND and
returns the pagenumber and the offset where key is present or could 
be inserted */
AM_ListSearch(fileDesc,attrType,attrLength,value,pageNum,pageBuf,indexPtr)
int fileDesc;
char attrType;
int attrLength;
char *value;
                                                            ...
                                                            can be inserted */
{
	...
	int nextPage; /* next page to be followed on the path from root to leaf*/
	int retval; /* return value */
	AM_NODE lhead,*lheader; /* local pointer to leaf header */

    /* initialise the headeers */	
	lheader = &lhead;
	
    /* get the root of the B+ tree */
	*pageNum = AM_HeadPageNum;
	errVal = PF_GetThisPage(fileDesc,*pageNum,pageBuf);
	AM_Check;
	
	bcopy(*pageBuf,lheader,AM_sn);
	if (lheader->attrLength != attrLength)
		return(AME_INVALIDATTRLENGTH);
		
	if(lheader->nextLeafPage == AM_NULL_PAGE){
		return(AM_ListSearchLeaf(*pageBuf,attrType,attrLength,value,indexPtr,lheader));
	}

	else{
		int tmp = *pageNum;
		//printf("%d\n",tmp);
		nextPage = AM_ListBinSearch(fileDesc,tmp,pageBuf,attrType,attrLength,value,lheader);
		//printf("%d\n",nextPage);

		errVal = PF_UnfixPage(fileDesc,*pageNum,FALSE);
		AM_Check;
		//AM_DumpLeafPages(fileDesc,0,attrType,attrLength);
...
/* Finds the place (index) from where the next page to be followed is got*/
AM_ListBinSearch(fileDesc,pageNum,pageBuf,attrType,attrLength,value,header)
int fileDesc;
int pageNum;
char **pageBuf; /* buffer where the page is found */
char attrType; 
...
AM_NODE *header;
{
	...
	currPageNum = header->nextLeafPage;
	errVal = PF_GetThisPage(fileDesc,currPageNum,pageBuf);
	AM_Check;

	bcopy(*pageBuf,header,AM_sn);
	if (header->attrLength != attrLength)
		return(AME_INVALIDATTRLENGTH);
	
	after = AM_Compare(*pageBuf + AM_sn, attrType,attrLength,value);
	
	int tmp;
	bcopy(value,&tmp,AM_si);
	//if(tmp == 1) printf("PN:%d C:%d\n",pageNum,after);

	errVal = PF_UnfixPage(fileDesc,currPageNum,FALSE);
	AM_Check;
	...
	if(after==-1){
		...
		return AM_ListBinSearch(fileDesc,currPageNum,pageBuf,
			attrType,attrLength,value,header);
	}
}
...
/* search a leaf node for the key- returns the place where it is found or can
be inserted */
AM_ListSearchLeaf(pageBuf,attrType,attrLength,value,indexPtr,header)
char *pageBuf; /* buffer where the leaf page resides */
char attrType;
int attrLength;
char *value; /* attribute value to be compared with */
...
AM_NODE *header;
{
	...
	while ((high - low ) > 1)
	{
		...
		/* compare value with the middle key */
		compareVal = AM_Compare(pageBuf + AM_sn + (mid - 1)*recSize,
		attrType,attrLength,value);
	...
	if ((high - low) == 0)
	{ 
		compareVal = AM_Compare(pageBuf+AM_sn+(low - 1)*recSize,
		attrType ,attrLength, value);
	...
	if ((high - low) == 1)
	{
		...
		{ 
			compareVal = AM_Compare(pageBuf+AM_sn+low*recSize,
			attrType, attrLength, value);
