...
/* Opens an index scan */
AM_ListOpenIndexScan(fileDesc,attrType,attrLength,op,value)
...
AM_NODE head,*header; /* local header */
int searchpageNum;
...
if (value == NULL)
  {
   ...
   AM_scanTable[scanDesc].nextpageNum = AM_HeadPageNum;
   AM_scanTable[scanDesc].nextIndex = 1;
   AM_scanTable[scanDesc].actindex = 1;
   errVal = PF_GetThisPage(fileDesc,AM_HeadPageNum,&pageBuf);
   AM_Check;

   bcopy(pageBuf + AM_sn + attrLength,&AM_scanTable[scanDesc].nextRecIdPtr,AM_ss);
   errVal = PF_UnfixPage(fileDesc,AM_HeadPageNum,FALSE);
   AM_Check;
   return(scanDesc);
  }
  
/* search for the pagenumber and index of value */
status = AM_ListSearch(fileDesc,attrType,attrLength,value,&pageNum,&pageBuf,&index);
...
switch(op) 
{
  case EQUAL : 
  {
    ...
    else
    {
      ...
      bcopy(pageBuf + AM_sn + (index - 1)*recSize + attrLength,
  ...
  case LESS_THAN : 
  {
    ...
    AM_scanTable[scanDesc].nextIndex = 1;
    AM_scanTable[scanDesc].actindex = 1;
    if (searchpageNum != AM_HeadPageNum)
    { 
      errVal = PF_GetThisPage(fileDesc,AM_HeadPageNum,&pageBuf);
      AM_Check;
    }
  ...
  case GREATER_THAN :
  {
    ...
    if ((index + 1) <= (header->numKeys))
    {
      ...
      bcopy(pageBuf + AM_sn + (index)*recSize + attrLength,
               &AM_scanTable[scanDesc].nextRecIdPtr,AM_ss);
    }
    else
    /* got to start from next leaf page */
      if (header->nextLeafPage != AM_NULL_PAGE)
      {
        ...
        errVal =PF_GetThisPage(fileDesc,header->nextLeafPage,&pageBuf);
        AM_Check;
        bcopy(pageBuf + AM_sn + attrLength,
               &AM_scanTable[scanDesc].nextRecIdPtr,AM_ss);
        errVal = PF_UnfixPage(fileDesc,header->nextLeafPage,FALSE);
        AM_Check;
      }
...
while(header->numKeys == 0)
  if(header->nextLeafPage == AM_NULL_PAGE)
  ...
  else
   {
    ...
    AM_scanTable[scanDesc].actindex = 1;
    bcopy(pageBuf,header,AM_sn);
    bcopy(pageBuf + AM_sn +  (AM_scanTable[scanDesc].nextIndex-1)*recSize
       ...
       if ((AM_scanTable[scanDesc].nextIndex + 1) <= (header->numKeys))
         {
          ...
          if (header->nextLeafPage == AM_NULL_PAGE)
          ...
          else
           {
            ...
            AM_Check;
            bcopy(pageBuf + AM_sn + header->attrLength,
               &AM_scanTable[scanDesc].nextRecIdPtr,AM_ss);
            bcopy(pageBuf,header,AM_sn);
            errVal = PF_UnfixPage(AM_scanTable[scanDesc].fileDesc
...
if (AM_scanTable[scanDesc].status != FIRST)
 {
  compareVal = AM_Compare(pageBuf + (AM_scanTable[scanDesc].nextIndex - 1)
                *recSize + AM_sn,AM_scanTable[scanDesc].attrType,
...
AM_ListCloseIndexScan(scanDesc)
...
/* Opens an index scan */
AM_BTreeOpenIndexScan(fileDesc,attrType,attrLength,op,value)
int fileDesc; /* file Descriptor */
...
AM_LeftPageNum = BTreeGetLeftPageNum(fileDesc);
...
/* returns the record id of the next record that satisfies the conditions
specified for index scan associated with scanDesc */
AM_BTreeFindNextEntry(scanDesc)
int scanDesc;/* index scan descriptor */
...
/* terminates an index scan */
AM_BTreeCloseIndexScan(scanDesc)
int scanDesc;/* scan Descriptor*/
