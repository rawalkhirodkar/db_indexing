...
AM_ListPrintLeafNode(pageBuf,attrType)
char *pageBuf;
char attrType;
{
short nextRec;
...
for (i = 1; i <= header->numKeys; i++)
  ...
  AM_ListPrintAttr(pageBuf + AM_sn + (i-1)*recSize,attrType,header->attrLength);
...
AM_ListDumpLeafPages(fileDesc,min,attrType,attrLength)
int fileDesc;
int min;
int attrLength;
char attrType;
...
printf("%d PAGE \n",AM_HeadPageNum);
PF_GetThisPage(fileDesc,AM_HeadPageNum,&pageBuf);
...
while(header->nextLeafPage != -1)
  {
   printf("PAGENUMBER = %d\n",pageNum);
   printf("PREVIOUSPAGENUMBER = %d\n",header->previousLeafPage);
   printf("KEYS = %d\n",header->numKeys);   
   AM_ListPrintLeafKeys(pageBuf,attrType);
...
AM_ListPrintAttr(bufPtr,attrType,attrLength)
char *bufPtr;
char attrType;
int attrLength;
{
...
AM_BTreePrintIntNode(pageBuf,attrType)
char *pageBuf;
char attrType;
{
int tempPageint;
...
for(i = 1 ; i <= (header->numKeys);i++)
  {
   AM_BTreePrintAttr(pageBuf + (i-1)*recSize + AM_sint + AM_si,attrType,
                 header->attrLength);
...
AM_BTreePrintLeafNode(pageBuf,attrType)
char *pageBuf;
char attrType;
...
for (i = 1; i <= header->numKeys; i++)
  ...
  AM_BTreePrintAttr(pageBuf + AM_sl + (i-1)*recSize,attrType,header->attrLength);
  bcopy(pageBuf + offset1 + header->attrLength,(char *)&nextRec,AM_ss);
  while (nextRec != 0)
    {
    bcopy(pageBuf + nextRec,(char *)&recId,AM_si);
...
AM_BTreeDumpLeafPages(fileDesc,min,attrType,attrLength)
int fileDesc;
int min;
int attrLength;
char attrType;
...
while(header->nextLeafPage != -1)
  {
   printf("PAGENUMBER = %d\n",pageNum);   
   AM_BTreePrintLeafKeys(pageBuf,attrType);
   errVal = PF_UnfixPage(fileDesc,pageNum,FALSE);
   AM_Check;
   pageNum = header->nextLeafPage;   
   errVal = PF_GetThisPage(fileDesc,pageNum,&pageBuf);
...
AM_BTreePrintAttr(bufPtr,attrType,attrLength)
char *bufPtr;
char attrType;
int attrLength;
...
AM_BTreePrintTree(fileDesc,pageNum,attrType)
int pageNum;
int fileDesc;
char attrType;
...
if (*tempPage == 'l')
  {
   printf("PAGENUM = %d\n",pageNum);
   AM_BTreePrintLeafKeys(tempPage,attrType);
   return;
  }
header = (AM_INTHEADER *)calloc(1,AM_sint);
...
for(i = 1; i <= (header->numKeys + 1); i++)
  {
   ...
   AM_BTreePrintTree(fileDesc,nextPage,attrType);
  }
printf("PAGENUM = %d",pageNum);
AM_BTreePrintIntNode(tempPage,attrType);
}