...
double cr1time = 0, cr2time = 0, cr3time = 0, cr4time = 0;	
	...
	/* LT_OP range queries on List MULTIPLE RECORDS */
	numrec= 0;
	sd = AM_ListOpenIndexScan(fd,INT_TYPE,sizeof(int),LT_OP,(char*) &val);
	...
	/* GT_OP range queries on List MULTIPLE RECORDS */
	numrec= 0;
	sd = AM_ListOpenIndexScan(fd,INT_TYPE,sizeof(int),GT_OP,(char*) &val);
	gettimeofday(&tim1, NULL);
	while((recnum=AM_ListFindNextEntry(sd))>= 0){
		numrec++;
	}
	...
	/* LE_OP range queries on List MULTIPLE RECORDS */
	numrec= 0;
	sd = AM_ListOpenIndexScan(fd,INT_TYPE,sizeof(int),LE_OP,(char*) &val);
	gettimeofday(&tim1, NULL);
	while((recnum=AM_ListFindNextEntry(sd))>= 0){
		numrec++;
	}
	...
	/* GE_OP range queries on List MULTIPLE RECORDS */
	numrec= 0;
	sd = AM_ListOpenIndexScan(fd,INT_TYPE,sizeof(int),GE_OP,(char*) &val);
	gettimeofday(&tim1, NULL);
	while((recnum=AM_ListFindNextEntry(sd))>= 0){
		numrec++;
	}
	...
	/// List PERFORMANCE ANALYSIS FOR CONCAT RECORDS ///
	...
	for (recnum=0; recnum < maxrecs; recnum+=1){
		for(i=0; i<10; i++){
			tmp = (recnum*10 + i);
			AM_ListInsertEntry(fd,INT_TYPE,sizeof(int),(char *)&tmp,rec);
			rec++;
		}
	}
	...
	/* LT_OP range queries on List MULTIPLE RECORDS */
	numrec= 0;
	sd = AM_ListOpenIndexScan(fd,INT_TYPE,sizeof(int),LT_OP,(char*) &val1);
	gettimeofday(&tim1, NULL);
	while((recnum=AM_ListFindNextEntry(sd))>= 0){
		numrec++;
	}
	...
	/* GT_OP range queries on List MULTIPLE RECORDS */
	numrec= 0;
	sd = AM_ListOpenIndexScan(fd,INT_TYPE,sizeof(int),GT_OP,(char*) &val2);
	gettimeofday(&tim1, NULL);
	while((recnum=AM_ListFindNextEntry(sd))>= 0){
		numrec++;
	}
	...
	/* LE_OP range queries on List MULTIPLE RECORDS */
	numrec= 0;
	sd = AM_ListOpenIndexScan(fd,INT_TYPE,sizeof(int),LE_OP,(char*) &val2);
	gettimeofday(&tim1, NULL);
	while((recnum=AM_ListFindNextEntry(sd))>= 0){
		numrec++;
	}
	...
	/* GE_OP range queries on List MULTIPLE RECORDS */
	numrec= 0;
	sd = AM_ListOpenIndexScan(fd,INT_TYPE,sizeof(int),GE_OP,(char*) &val1);
	gettimeofday(&tim1, NULL);
	while((recnum=AM_ListFindNextEntry(sd))>= 0){
		numrec++;
	}
